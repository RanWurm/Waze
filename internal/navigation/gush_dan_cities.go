...
// InitializeBidirGushDanCities sets up entry points for major cities in Gush Dan (for BidirEntryPointRouter)
func InitializeBidirGushDanCities(router *BidirEntryPointRouter) {
	g := router.Graph
	epm := router.EntryPointManager
	...
	log.Printf("Bidir Entry point system initialized: %d cities, %d nodes mapped", len(epm.Cities), totalNodes)
}
